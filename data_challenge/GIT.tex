...
\begin{enumerate}
  ...
  \item \textbf{Automatic synchronisation.} If a
        ...
        requirements---no manual \texttt{pip install} is needed.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Python virtual environments: what they are, why they matter, and
        ...
        platforms using the standard \texttt{pip} and \texttt{venv}
        toolchain maintained by the Python Software Foundation.
  \item \textbf{\texttt{uv}}: a modern, high-performance alternative to
        \texttt{pip} and \texttt{venv}, developed by Astral. \texttt{uv}
        offers dramatically faster installs, a global package cache,
        reproducible lockfiles, and native \texttt{pyproject.toml}
        support. Because \texttt{uv} is maintained by a commercial
        entity rather than a foundation-backed community project, knowing
        both tools remains the prudent professional position until the
        ecosystem settles.
  \item \textbf{Level~1} scripting: writing \texttt{ActivateEnv.bat} /
        \texttt{ActivateEnv.sh} and \texttt{DAIR3.bat} / \texttt{DAIR3.sh}
        as reusable launchers for \texttt{pip}-managed environments, and
        \texttt{ActivateUVEnv.bat} / \texttt{ActivateUVEnv()} as their
        \texttt{uv}-managed counterparts with automatic environment
        creation and dependency synchronisation.
